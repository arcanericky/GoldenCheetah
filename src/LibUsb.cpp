...
int LibUsb::initialise_ant(libusb_device *device, const struct libusb_device_descriptor &desc) {
...
int LibUsb::open_device(libusb_device *device, const struct libusb_device_descriptor &desc) {
...
int LibUsb::write(char *buf, int bytes)
{
    ...
    int transferred;
    int rc = libusb_bulk_transfer(handle, writeEndpoint, (unsigned char *)buf, bytes,
            &transferred,
